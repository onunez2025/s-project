...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, DashboardComponent, ProjectDetailComponent, UserManagementComponent, AreaManagementComponent, BiDashboardComponent, KanbanBoardComponent, LoginComponent, ProfileComponent, ManualComponent],
  template: `
          ...
          <div class="h-20 flex items-center px-8 border-b border-slate-800/50 bg-[#0F172A] shrink-0">
            <div class="flex items-center gap-2">
              ...
              <span class="text-2xl font-bold tracking-tight text-white">
                Project<span class="text-blue-500">.</span>
              </span>
            </div>
          </div>
        ...
        <main class="flex-1 flex flex-col min-w-0 overflow-hidden relative h-full">
          ...
          <header class="md:hidden bg-[#0F172A] border-b border-slate-800 h-16 flex items-center px-4 justify-between shadow-md z-30 shrink-0">
             ...
             <div class="flex items-center gap-3">
               ...
               <button (click)="dataService.logout()" class="text-slate-400">
          ...
          <div class="flex-1 overflow-auto p-4 sm:p-8 relative custom-scrollbar">
             @switch (currentView()) {
               @case ('BI') {
                  <app-bi-dashboard 
                     (selectProject)="goToDetail($event)"
                     (goToProjects)="currentView.set('LIST')"
                     (goToManual)="goToManual('projects')">
                  </app-bi-dashboard>
               }
               @case ('LIST') {
                 <app-dashboard 
                    (onSelect)="goToDetail($event)"
                    (goToManual)="goToManual('projects')"
                 ></app-dashboard>
               }
               @case ('DETAIL') {
                 <app-project-detail
                    ...
                    (back)="currentView.set('LIST')"
                    (goToManual)="goToManual($event)"
                 ></app-project-detail>
               }
               @case ('KANBAN') {
               ...
               @case ('MANUAL') {
                 <app-manual [section]="manualSection()"></app-manual>
               }
             }
          </div>
        </main>
...
export class AppComponent {
  ...
  currentView = signal<ViewState>('BI'); // Default to BI
